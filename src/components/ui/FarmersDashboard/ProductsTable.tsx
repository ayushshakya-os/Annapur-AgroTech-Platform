...
import { useToggleBiddable } from "@/hooks/api/FarmersDashboard/useToggleBiddable";
...
export default function ProductsTable({
  ...
  products: Product[];
  onToggleBiddable: (p: Product) => void; // kept for immediate UI updates if parent maintains local state
}) {
  const [editing, setEditing] = useState<Product | null>(null);
  const [isModalOpen, setModalOpen] = useState(false);

  const { mutate: toggleBiddable, isPending: isToggling } = useToggleBiddable();
                ...
                <tr key={p._id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="h-15 w-15 rounded-md overflow-hidden bg-gray-100">
                        {p.image ? (
                          <img
                            src={p.image}
                            alt={p.name}
                  ...
                  <td className="px-4 py-3">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        disabled={isToggling}
                        onChange={() => {
                          // Immediate UI via parent handler (if using local state)
                          onToggleBiddable?.(p);
                          // Persist via API
                          toggleBiddable({
                            id: p._id,
                            isBiddable: !p.isBiddable,
                          });
                        }}
                      />
                      {/* track */}
